...
void multMatMat(MatRow A, MatRow B, int n, MatRow C)
{
    ...
    // Armazenar em variáveis economiza o cálculo nos laços
    int n_mod = n % UNRL;
    int n_minus_n_mod = n - n_mod;


    for (i = 0; i < n; ++i) {
        for (j = 0; j < n_minus_n_mod; j += UNRL) {
            ...
            // Acumuladores economizam escritas em C[].
            C[i*n + j] += sum0;
            C[i*n + j + 1] += sum1;
            C[i*n + j + 2] += sum2;
            C[i*n + j + 3] += sum3;
    ...
    // Residuais do loop unrolling
    for (i = 0; i < n; ++i) {
        for (j = n_minus_n_mod; j < n; ++j) {
            for (k = 0; k < n; ++k) {
